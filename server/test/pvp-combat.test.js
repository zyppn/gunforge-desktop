...
// ---------- 1. shotgun spread: damage falls off with range ----------
{
  const sample = (dist, trials) => {
    ...
    for(let i = 0; i < trials; i++){
      ...
      input(r, 'A', 0);
      r.fireT.set('A', -1);
      r.tryFire('A', a, r.inputs.get('A'), 0);
      total += (100 - b.hp);
    }
    return total / trials;
  };
...
// ---------- 2. deadeye crit rate lands near 12% ----------
{
  ...
  for(let i = 0; i < N; i++){
    ...
    face(a, b, 6); input(r, 'A', 0); r.fireT.set('A', -1);
...
// ---------- 6. vampiric heals the shooter, capped at 100 ----------
{
  ...
  a.hp = 99; r.fireT.set('A', -1); r.tryFire('A', a, r.inputs.get('A'), 0);
...
// ---------- 8. range limit ----------
{
  ...
  const b = join(r, 'B', 'FAR', {}, 'm17');
  a.x = 1; a.z = 10; b.x = 1 + 70; b.z = 10;   // 70m, past maxRange 60
  input(r, 'A', 0); r.fireT.set('A', -1);
  r.tryFire('A', a, r.inputs.get('A'), 0);
  console.log('\nRANGE    target at 70m took ' + (100 - b.hp).toFixed(1));
  check('nothing connects past maxRange', b.hp === 100);
}
